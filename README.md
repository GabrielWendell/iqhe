...
**Current release:** `v0.6.0` — static-topology reproducibility baseline.

The release verifies the Hamiltonian core, bulk topology, FHS/interband-Kubo agreement, ribbon edge
localization, and measurable static bulk-boundary correspondence. It regenerates manuscript Figures 1–6
from source scripts and executes the Stage 1–4 teaching notebooks.

The following are intentionally not yet claimed as validated outputs: time-domain edge-projected
wave-packet propagation, norm-conservation analysis, bond currents, edge leakage, packet/group-velocity
agreement, and controlled defect routing. Those require their own dynamics stage.
...
The release smoke test runs the structural checks, Stage 1–4 validations, tests, all production teaching
notebooks, and a clean regeneration of Figures 1–6. Details are in
...
python scripts/create_release_archive.py --tag v0.6.0
```
...
6. `05_chiral_dynamics.ipynb` — reserved for a future validated dynamics implementation.
7. `06_velocity_and_defect.ipynb` — reserved for a future velocity/defect workflow.
